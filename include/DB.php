...
class DB
{
	...
	public function __construct($config = [])
	{
		...
		{
			throw new Exception("DB error: Can not connect to Database with PDO. " . $e->getMessage());
		}
	...
	/**
	 * Defines 'Order By Rand' operation
	 *
	 * @param string empty
	 * @param string empty
	 * @return $this
	...
	public function insert($data = [])
	{
		...
		if (is_null($this->table))
			throw new Exception('DB Hatası: INSERT işlemi yapılacak tablo seçilmedi.');
	...
	{
		if (null !== $this->error)
			throw new Exception('DB Hatası: ' . $this->error);
	}
